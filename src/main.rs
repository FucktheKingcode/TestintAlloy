use alloy::{
    contract::{ContractInstance, Interface}, dyn_abi::DynSolValue, network::{Ethereum, EthereumWallet, TransactionBuilder}, primitives::{address, U256}, providers::{Provider, ProviderBuilder}, rpc::types::TransactionRequest, signers::local::PrivateKeySigner, transports::http::{Client, Http}
};
use eyre::Result;
use serde_json;
...
async fn main() -> Result<()> {
    ...
    let signer: PrivateKeySigner = "7b3db5b198fc6d41a61d8d3550630b5b9b2e77c4c672f135e4a6e7991d3b557e".parse().expect("should parse private key");
    let wallet = EthereumWallet::from(signer);

    // Create a provider with the wallet.
    let rpc_url = "https://public-en-baobab.klaytn.net".parse()?;
    let provider =
    ...
    let tx = TransactionRequest::default()
        .with_to(address!("a214166C1302D0FFEaee7Ca54F65CA47f88Cbd5F"))
